...
data class QuizEntity(
    ...
    fun validate() {
        if (createdBy == null) throw EntityValidationException(QuizEntity::class, "CreatedBy is null in $id")
        if (answers == null || answers!!.isEmpty()) throw EntityValidationException(QuizEntity::class, "No answers for quiz $id")
        if (topics == null || topics!!.isEmpty()) throw EntityValidationException(QuizEntity::class, "No topics for quiz $id")
    }
}
...
@EnableScan
interface QuizRepository : CrudRepository<QuizEntity, String> {

    @CacheEvict(value = "quiz", key = "#result?.id")
    fun save(quiz: QuizEntity): QuizEntity

    override fun findAll(): List<QuizEntity>

    @Cacheable(value = "quiz")
    fun findOneById(id: String): QuizEntity?

    fun findByTopicsContaining(topics: String): List<QuizEntity>

    fun findTop50ByIdGreaterThan(randomKey: String): List<QuizEntity>

    fun findTop50ByIdLessThan(randomKey: String): List<QuizEntity>
}
...
class QuizTopicRepository(
    ...
    fun saveTopics(quiz: QuizEntity) {
    ...
    internal fun findTopics(quiz: QuizEntity): List<QuizTopic> {
    ...
    private fun generateTopics(quiz: QuizEntity): List<QuizTopic> {
